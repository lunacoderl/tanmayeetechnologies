  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        ...
        {/* Tab switcher */}
        <div className="flex items-center bg-slate-100 p-1 rounded-xl text-[11px] sm:text-xs font-bold self-start sm:self-auto overflow-x-auto max-w-full">
          <button
            type="button"
            onClick={() => setActiveTab('requests')}
            className={`px-2.5 sm:px-3.5 py-1 sm:py-1.5 rounded-lg transition-colors whitespace-nowrap ${
              activeTab === 'requests'
        ...
        <div className="space-y-3 sm:space-y-4">
            ...
            <div className="flex items-center gap-1.5 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0 text-xs font-bold">
              {['ALL', 'PENDING', 'IN_PROGRESS', 'COMPLETED'].map((st) => (
                <button
                  ...
                  className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-xl text-[10px] sm:text-xs transition-colors whitespace-nowrap ${
                    statusFilter === st
          ...
          <div className="space-y-3">
            {filteredRequests.map((req) => (
              <div
                key={req.id}
                className="bg-white p-3.5 sm:p-5 rounded-xl sm:rounded-2xl border border-slate-200 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-3 sm:gap-4"
              >
                <div className="space-y-2 lg:max-w-2xl">
                  <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                    <span className="font-mono text-xs font-black text-brand-600 bg-brand-50 px-2 py-0.5 rounded-md border border-brand-100">
                      {req.request_number}
                    </span>
                    ...
                    <span
                      className={`text-[9px] sm:text-[10px] font-extrabold px-2 py-0.5 rounded-md uppercase ${
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                    ...
                    <div>
                      ...
                      <a
                        ...
                        className="font-bold text-brand-600 hover:underline flex items-center gap-1 text-[11px] sm:text-xs"
                      >
                  ...
                  {req.equipment_details && (
                    <div className="text-[11px] sm:text-xs bg-slate-50 p-2 sm:p-2.5 rounded-xl border border-slate-100 text-slate-700">
                      <span className="font-bold text-slate-900 block text-[9px] sm:text-[10px] uppercase tracking-wider mb-0.5">
                ...
                <div className="flex flex-col sm:flex-row lg:flex-col items-stretch sm:items-center lg:items-end gap-2 shrink-0 pt-3 lg:pt-0 border-t lg:border-t-0 border-slate-100">
                  <select
                    value={req.status}
                    onChange={(e) => handleStatusUpdate(req.id, e.target.value)}
                    className="w-full sm:w-auto text-xs font-bold bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5 focus:outline-none focus:border-brand-500"
                  >
                  ...
                  <a
                    ...
                    className="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs px-3.5 py-2 rounded-xl shadow-xs transition-colors"
                  >
